...
jQuery(document).ready(function ($) {
    ...
    jQuery('.search-box').focus(function () {
        jQuery('.search-area input').css('width', '162px');
        jQuery('.search-area input').css('padding-left', '10px');
        jQuery('.search-area button').css('display', 'block');
        jQuery('#search-label').empty();
        jQuery('#search-label').css('left', '70px');
    });
    jQuery('.search-box').focusout(function () {
        jQuery('.search-area input').css('width', '42px');
        jQuery('.search-area input').css('padding', '0');
        jQuery('.search-area button').css('display', 'none');
        jQuery('#search-label').prepend('Procurar');
        jQuery('#search-label').css('left', '0');
    });
    ...
    if (subMenuLeft) {
        subMenuLeft.affix({
            offset: {
                top: 150,
                bottom: (jQuery(document).innerHeight() - jQuery('.row-after-art-all').offset().top)
            }
        });
            ...
            if (filter_items)
                filter_items.each(function () {
                    jQuery(this).on('click', function () {
                        ...
                        if (tag)
                            window.location.href = window.location.href.split("?")[0].split("#")[0] + '?tag=' + tag;
                        else
...
jQuery.getScript("/templates/template-camoes-home/js/jquery.mobile.custom.min.js")
...
//Function tags
jQuery(document).on('click', '.tagid', function () {
    str = jQuery(this).attr("id");
    jQuery('#noticias').fadeTo(200, .4);
    //Delete cookie
    document.cookie = "tag=; expires=Thu, 01 Jan 1970 00:00:00 UTC";
    //Insert cookie
    document.cookie = "tag=" + str;
    jQuery.ajax({
        url: "getuser.php?tagid=" + str,
    })
         .done(function (data) {
    ...
    jQuery("#categorias li").removeClass("current active");
    jQuery(this).parent('li').addClass("current active");
    jQuery('#noticias').fadeTo(200, 1);
    return false;
});
jQuery(document).on('click', '.tagtodos', function () {
    str = jQuery(this).attr("id");
    jQuery('#noticias').fadeTo(200, .4);
    //Delete cookie
    document.cookie = "Pais=; expires=Thu, 01 Jan 1970 00:00:00 UTC";
    //Insert cookie
    document.cookie = "Pais=" + str;
    jQuery.ajax({
        url: "gettodos.php",
    })
         .done(function (data) {
...
jQuery(document).on('click', '.tagtudo', function () {
    str = jQuery(this).attr("id");
    jQuery('#noticias').fadeTo(200, .4);
    //Delete cookie
    document.cookie = "tag=; expires=Thu, 01 Jan 1970 00:00:00 UTC";
    jQuery.ajax({
        url: "gettudo.php?tagid=" + str,
    })
         .done(function (data) {
             document.getElementById("noticias").innerHTML = data;
         })
...
jQuery(document).on('click', '.tagregiao', function () {
    str = jQuery(this).attr("id");
    jQuery('#noticias').fadeTo(200, .4);
    //Delete cookie
    document.cookie = "Pais=; expires=Thu, 01 Jan 1970 00:00:00 UTC";
